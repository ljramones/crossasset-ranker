"""Standalone CLI for drawdown-risk simple classifier baselines."""
...
from experiments.drawdown_risk_classifier_experiment import run_drawdown_risk_classifier_experiment
...
def infer_feature_columns(frame: pd.DataFrame) -> list[str]:
    ...
    excluded.update({column for column in frame.columns if column.startswith("future_max_drawdown_")})
    excluded.update({column for column in frame.columns if column.startswith("target_drawdown_event_")})
...
def main() -> None:
    ...
    if args.dry_run:
        print("Drawdown-risk classifier runner dry run only.")
        print("No data will be loaded. No outputs will be written.")
        return

    required = ["input_csv", "output_dir", "target_column", "model_name"]
    ...
    summary_path = output_dir / f"drawdown_risk_classifier_summary_{timestamp}.csv"
    fold_details_path = output_dir / f"drawdown_risk_classifier_fold_details_{timestamp}.csv"
    oof_path = output_dir / f"drawdown_risk_classifier_oof_artifacts_{timestamp}.csv"
    report_path = output_dir / f"drawdown_risk_classifier_report_{timestamp}.md"
    metadata_path = output_dir / f"drawdown_risk_classifier_metadata_{timestamp}.json"
    ...
    report_lines = [
        "# Drawdown-Risk Classifier Experiment Report",
        "",
