"""CLI wrapper for the standalone volatility-quantile overlay experiment."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the standalone vol-quantile overlay experiment.")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--dry-run", action="store_true", help="Validate command shape without loading data or writing outputs.")
    mode.add_argument("--execute", action="store_true", help="Run the real standalone vol-quantile overlay experiment.")
    parser.add_argument("--input-csv", help="Prepared feature frame CSV.")
    ...
    parser.add_argument("--model-name", default="vol_quantile_overlay_hard_veto")
...
def _build_output_paths(output_dir: Path, *, timestamp: str) -> dict[str, Path]:
    return {
        "summary": output_dir / f"vol_quantile_overlay_experiment_summary_{timestamp}.csv",
        "fold_details": output_dir / f"vol_quantile_overlay_experiment_fold_details_{timestamp}.csv",
        "audit_artifacts": output_dir / f"vol_quantile_overlay_experiment_audit_artifacts_{timestamp}.csv",
        "matched_nulls": output_dir / f"vol_quantile_overlay_experiment_matched_nulls_{timestamp}.csv",
        "report_markdown": output_dir / f"vol_quantile_overlay_experiment_report_{timestamp}.md",
        "metadata": output_dir / f"vol_quantile_overlay_experiment_metadata_{timestamp}.json",
    }
...
def _build_markdown_report(
    *,
    ...
    lines = [
        "# Volatility-Quantile Overlay Experiment Report",
        "",
...
def main(argv: list[str] | None = None) -> Path:
    ...
    if args.dry_run:
        print("Volatility-quantile overlay runner dry run only.")
